...
export default function ForgotPassword({ onBack, email, shared }: Props): React.JSX.Element {
  ...
  if (step === 1) {
    return (
      <Form
        {...shared}
        onSubmit={(m, e) => {
          e.preventDefault(); // prevent modalClose on $submitEnd
          return apiForgot(m).then(() => {
  ...
  return (
    ...
    <Form
      ...
      onSubmit={(m, e) => {
        e.preventDefault(); // prevent modalClose on $submitEnd
        return apiResetPwd({ ...m, email: lastEmail }).then(onBack);
      }}
      autoComplete={false}
    >
      <h2>New Password</h2>
...
// todo button 'resend code'
// todo recheck handling error
